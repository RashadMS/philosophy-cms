...
const transporter = nodemailer.createTransport({
  ...
  // الإضافات الضرورية لبيئة Render:
  family: 4,                // حل مشكلة IPv6 (ENETUNREACH)
  connectionTimeout: 15000,  // إذا فشل الاتصال يفشل بسرعة بدل أن يظل الزر عالقاً
  greetingTimeout: 10000,
  tls: {
    rejectUnauthorized: false,
    servername: 'smtp.gmail.com' // تأكيد اسم السيرفر لتجاوز قيود SSL
  }
});
/**
...
export async function sendVerificationEmail(email, verificationToken, userName) {
  // التعديل هنا لضمان تنظيف الرابط من أي علامات مائلة إضافية
  const baseUrl = process.env.APP_URL ? process.env.APP_URL.replace(/\/$/, "") : 'http://localhost:3000';
